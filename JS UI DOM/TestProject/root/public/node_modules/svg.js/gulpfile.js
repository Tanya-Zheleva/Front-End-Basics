...
gulp.task('unify', ['clean'], function () {
  ...
  .pipe(wrapUmd({ src: 'src/umd.js' })).pipe(header(headerLong, { pkg: pkg })).pipe(trim({ leading: false })).pipe(chmod(420)).pipe(gulp.dest('dist')).pipe(size({ showFiles: true, title: 'Full' }));
});
...
gulp.task('minify', ['unify'], function () {
  return gulp.src('dist/svg.js').pipe(uglify()).pipe(rename({ suffix: '.min' })).pipe(size({ showFiles: true, title: 'Minified' })).pipe(header(headerShort, { pkg: pkg })).pipe(chmod(420)).pipe(gulp.dest('dist')).pipe(size({ showFiles: true, gzip: true, title: 'Gzipped' }));
});
